        ...
        for (rotation = 0; rotation < 4; rotation++)
            for (i = 0; i < 256; i++)
                snefru512_rotated[rotation][index][i] =
                    (
                        (
                            (uint32)snefru512_sboxes[index][i]
                        >>
                            (rotation * 8)
                        )
                    |
...
static int
snefru_hash(fingerprint_ty *p, unsigned char *h, size_t h_len)
{
    snefru_ty       *s;
    int             len[8];
    int             i;
...
static void
snefru_sum(fingerprint_ty *p, char *obuf, size_t obuf_len)
{
    int             j;
    char            *cp;
