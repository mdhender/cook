...
#include <common/str/re.h>
#include <common/stracc.h>


int
str_re_match(string_ty *formal, string_ty *actual,
...
string_ty *
str_re_substitute(string_ty *lhs, string_ty *rhs, string_ty *actual,
    void (*error_callback)(const char *), int how_many_times)
{
#ifdef HAVE_REGCOMP
    static stracc   sa;
    regex_t         preg;
    ...
    while (ip < ip_end)
    {
        ...
        for (cp = rhs->str_text; *cp; ++cp)
        {
            ...
            case '&':
                rm = &match[0];
                sa_chars(&sa, ip + rm->rm_so, rm->rm_eo - rm->rm_so);
                break;
