...
#       Copyright (C) 1997, 1998, 2003, 2007, 2008 Peter Miller
#
...
#
# Specifically: a rule which depends on one leaf and one derived.
# Make sure is done in the right order.
#
cat > Howto.cook << 'fubar'
set time-adjust;
compound: leaf non-leaf { cat [need] > [target]; }
non-leaf: fred { cat [need] > [target]; }
fred: { date > [target]; }
fubar
if test $? -ne 0 ; then no_result; fi
...
#
test -f compound || fail
test -f non-leaf || fail
test -f fred || fail
