...
$bin/c_incl -nc -ns test.in -o test.out -qfn
...
#
$bin/c_incl -nc -nsri -ns test.in -qfn > test.out 2>&1
...
$bin/c_incl -nc -I. -Ibl -rlp=. -rlp=bl -ns test.in \
        -o test.out -qfn -prefix="prefix=" -en
if test $? -ne 0 ; then fail; fi
...
$bin/c_incl -nc -I. -Ibl -rlp=. -slp bl '[bl]' -ns test.in -o test.out -qfn
if test $? -ne 0 ; then fail; fi
