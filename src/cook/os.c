 ...
 *	Copyright (C) 1995-2001, 2004 Peter Miller;
 *	All rights reserved.
 *
	...
	long		nsec;
{
	...
	else if (nsec >= (60 * 60))
	{
		...
		buffer =
			...
			(
				scp,
				i18n("$number1 hours $number2 minutes")
			);
	}
	else if (nsec >= 60)
	{
		...
		buffer =
			...
			(
				scp,
				i18n("$number1 minutes $number2 seconds")
			);
	}
	else
	{
...
static int utimes4cygwin _((string_ty *path, struct utimbuf *ut));

static int
utimes4cygwin(path, ut)
	string_ty	*path;
	struct utimbuf	*ut;
{
	string_ty	*fullpath;
