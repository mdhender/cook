 ...
 *
 * MANIFEST: functions to manipulate if statement nodes
 */

#include <expr.h>
#include <opcode/label.h>
#include <opcode/list.h>
#include <opcode/goto.h>
#include <opcode/jmpf.h>
...
stmt_ty *
stmt_if_new(condition, then_clause, else_clause)
	expr_ty		*condition;
	stmt_ty		*then_clause;
	stmt_ty		*else_clause;
{
	...
	this->else_clause =
		(else_clause ? stmt_copy(else_clause) : (stmt_ty *)0);
