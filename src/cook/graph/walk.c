 ...
 *
 * MANIFEST: functions to perform a post-order traversal of a dependency graph
 */
...
char *
graph_walk_status_name(n)
	graph_walk_status_ty n;
{
	switch (n)
	{
...
static void implications_of_file _((graph_recipe_list_nrc_ty *, graph_file_ty *,
	int));
	...
	int		uptodate;
{
	...
	for (k = 0; k < gfp->output->nrecipes; ++k)
	{
		...
		trace(("walked %s:%d recipe (%ldth of %ld files)\n",
			(grp2->rp->pos.pos_name ?
				grp2->rp->pos.pos_name->str_text : ""),
	...
	)
	{
		...
		scp = sub_context_new();
		sub_var_set(scp, "File_Name", "%S", gfp->filename);
...
static void excuse_me _((symtab_ty *, string_ty *, void *, void *));

static void
excuse_me(stp, key, data, aux)
	symtab_ty	*stp;
	string_ty	*key;
	void		*data;
	void		*aux;
{
	...
	(
		gfp->primary_target
	&&
		gfp->input_satisfied < gfp->input->nrecipes
	)
	{
		sub_context_ty	*scp;
...
static void is_it_a_leaf _((symtab_ty *, string_ty *, void *, void *));

static void
is_it_a_leaf(stp, filename, data, aux)
	symtab_ty	*stp;
	string_ty	*filename;
	void		*data;
	void		*aux;
...
static void implications_of_recipe _((graph_recipe_list_nrc_ty *,
	graph_recipe_ty *, int));

static void
implications_of_recipe(walk, grp, uptodate)
	graph_recipe_list_nrc_ty *walk;
	graph_recipe_ty	*grp;
	int		uptodate;
{
...
static graph_walk_status_ty graph_walk_inner _((graph_ty *gp,
	graph_walk_status_ty (*func)(graph_recipe_ty *, graph_ty *), int));

static graph_walk_status_ty
graph_walk_inner(gp, func, nproc)
	graph_ty	*gp;
	graph_walk_status_ty (*func)_((graph_recipe_ty *, graph_ty *));
	int		nproc;
{
	graph_recipe_list_nrc_ty walk;
	...
	while (walk_pos < walk.nrecipes || itp->load > 0)
	{
		...
		if (desist_requested())
		{
			...
			if (itp->load > 0 && !option_test(OPTION_SILENT))
			{
				...
				(
					scp,
				       i18n("waiting for outstanding processes")
				);
				sub_context_delete(scp);
			}
		...
		while (itp->load < nproc && walk_pos < walk.nrecipes)
		{
			 ...
			 */
			if (grp->single_thread && status2 !=
				graph_walk_status_wait)
			{
				string_list_remove_list
				(
					&single_thread,
					grp->single_thread
				);
