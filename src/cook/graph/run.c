 ...
 *	Copyright (C) 1997-1999, 2001, 2003, 2004 Peter Miller;
 *	All rights reserved.
 *
...
static string_ty *host_binding_round_robin _((opcode_context_ty *,
	string_list_ty  *));

static string_ty *
host_binding_round_robin(ocp, slp)
...
i18n("\"$filename\" is out of date because the \"forced\" flag is set (reason)")
	...
	if (fp_ingredients_fingerprint_differs(target1, ingr_fp))
	{
		...
		(
	    	    &grp->rp->pos,
	    	    scp,
i18n("\"$filename\" is out of date because the ingredients changed (reason)")
		);
		sub_context_delete(scp);
	    }
	}
	...
	if (gfp2->done && gftp2->edge_type != edge_type_exists)
	{
		...
		if (show_reasoning)
		{
		    ...
		    sub_var_set(scp, "File_Name2", "%S", gfp2->filename);
		    error_with_position
		    (
			&grp->rp->pos,
			scp,
...
now younger (reason)")
	...
	if (grp->rp->out_of_date)
	{
		...
		for (k = 0; k < grp->output->nfiles; ++k)
		{
		    ...
		    {
			scp = sub_context_new();
			sub_var_set(scp, "File_Name", "%S", gfp2->filename);
