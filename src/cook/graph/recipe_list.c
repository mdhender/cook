 ...
 *
 * MANIFEST: functions to manipulate graph recipe lists
 */
...
void
graph_recipe_list_append(grlp, grp)
	graph_recipe_list_ty *grlp;
	graph_recipe_ty	*grp;
{
	size_t		j;

	trace(("graph_recipe_list_append(grlp = %8.8lX, \
grp = %8.8lX)\n{\n"/*}*/,
		(long)grlp, (long)grp));
	for (j = 0; j < grlp->nrecipes; ++j)
	{
...
void
graph_recipe_list_append_list(to, from)
	graph_recipe_list_ty *to;
	graph_recipe_list_ty *from;
{
	size_t		j;

	trace(("graph_recipe_list_append_list(to = %8.8lX, \
from = %8.8lX)\n{\n"/*}*/,
		(long)to, (long)from));
...
void
graph_recipe_list_nrc_append(grlp, grp)
	graph_recipe_list_nrc_ty *grlp;
	graph_recipe_ty	*grp;
{
	size_t		j;

	trace(("graph_recipe_list_nrc_append(grlp = %8.8lX, \
grp = %8.8lX)\n{\n"/*}*/,
		(long)grlp, (long)grp));
	for (j = 0; j < grlp->nrecipes; ++j)
	{
...
void
graph_recipe_list_nrc_append_list(to, from)
	graph_recipe_list_nrc_ty *to;
	graph_recipe_list_nrc_ty *from;
{
	size_t		j;

	trace(("graph_recipe_list_nrc_append_list(to = %8.8lX, \
from = %8.8lX)\n{\n"/*}*/,
		(long)to, (long)from));
	for (j = 0; j < from->nrecipes; ++j)
...
void
graph_recipe_list_nrc_delete(grlp)
	graph_recipe_list_nrc_ty *grlp;
{
